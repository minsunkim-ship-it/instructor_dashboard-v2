...
중요:
- T1은 baseline schema 확인 역할만 가진다. prisma/schema.prisma는 수정하지 마라.
- Group 1의 목적은 T2(상세 패널 UI) + T3(만족도 작성 UI) + page.tsx 연결이다.
- page.tsx의 담당 그룹은 Group 1이다.
...
중요:
- Group 2의 목적은 T4(GET /api/status)와 T9(Fallback 배너 UI)다.
- T9는 배너 컴포넌트와 표시 계약만 제공한다.
- src/app/page.tsx는 수정하지 마라. page.tsx 연결은 Group 1 또는 마지막 T5 통합 단계에서 한다.
...
- src/app/api/refresh/route.ts 연결은 하지 마라. 그건 마지막 T5 통합 단계 전용이다.
...
2. Group 2의 FallbackBanner를 page.tsx에 최소 범위로 연결
...
- POST /api/refresh
- GET /api/status
- page.tsx에서 fallback/status wiring 확인
