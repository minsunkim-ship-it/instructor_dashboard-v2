...
- Wave 1 태스크 의미와 완료 기준: `11_wave1_tasks.md`
- 그룹 경계와 파일 담당 규칙: `12_parallel_bundle_guardrails.md`
- 시작 전 / `T5` 진입 전 체크: `14_wave1_preflight_checklist.md`
...
- 이번 웨이브는 **결과 품질 우선**으로 운영한다.
  - 문서 계약 준수와 최종 결과 정합성이 1순위다.
  - validated-plan, TDD, 기타 프로세스 도구는 수단이지 목적이 아니다.
- 모든 그룹은 **fresh 구현 원칙**을 따른다.
  - `main HEAD`를 기준으로 시작한다.
  - 과거 시도(`*-attempt*` branch 등)나 외부 branch를 근거로 구현하지 않는다.
- 이번 웨이브의 demo 대상은 **내부 팀 운영 의사결정용**이다.
  - 외부 세일즈/홍보용 데모를 가정하지 않는다.
- blocker는 **명시 + 가능한 범위 진행 후 최종 보고**를 기본 원칙으로 한다.
  - 즉시 중단하지 않는다.
  - 자동 우회 구현도 하지 않는다.
  - 허용되지 않은 파일 수정이나 공통 고정 항목 변경이 필요하면 blocker로 올린다.
...
- `T5` 단계에서는 구현 실패와 외부 source/runtime 실패를 구분해 보고한다.
  - 구현 실패: wiring 누락, 응답 구조 불일치, 타입/빌드 실패, 문서 계약 위반
...
- 아래 항목은 Group 1~3 시작 전에 먼저 확인한다.
  ...
  - `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, `GMAIL_ACCOUNT_EMAIL`
